_chronometer = function() {
	...
	this.settings['showRemaining'] = false;
	
	this.settings['remaining'] = null;
...
_chronometer.prototype.showRemaining = function(showRemaining)
{
	if (showRemaining) {
		this.settings['showRemaining'] = true;
		this.settings['remaining'] = showRemaining;
	}
	else {
		this.settings['showRemaining'] = false;
	}
...
_chronometer.prototype.start = function()
{
	...
	(function(self) {         	// replacement for bind()
         return function() {   
             self._tick(); 
         }
...
_chronometer.prototype._tick = function()
{
	...
	if (this.lastDiff && (Math.floor(this.lastDiff) - Math.floor(diff))!=0) {
		...
		if (this.settings['showRemaining']) {
			var progress = Math.min(1, Math.max(0, 1-(this.settings['remaining'] - diff)/this.settings['remaining']));
			this.target.find('div.progress div.progressbar').css('width', progress*100 + "%");
			
			var p = this.target.find('div.progress div.progressbar');
			var marks = [0, 25, 50, 75, 90];
			for (var i=0; i<marks.length; i++) {
				if (marks[i]<(progress*100)) {
					p.removeClass("m"+marks[i]+"p");
				}
				else {
					p.addClass("m"+marks[i]+"p");
					break;
				}
			}
		}
	...
	if (this.settings['showRemaining']) {
		if (this.settings['remaining'] - diff>0) {
			this.render(this.settings['remaining'] - diff);
		}
		else {
			this.target.addClass('warning');
			this.render(Math.abs(this.settings['remaining'] - diff));
		}
	}
	else {
		this.render(diff);
